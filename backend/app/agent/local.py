...
def infer(message: str):
    ...
    low=message.lower()
    if oid and any(x in low for x in ("cancel","fee")): return [("lookup_records", LookupQuery(record_type="order",record_id=oid,include_related=True))]
    if oid and any(x in low for x in ("credit","late","pickup","carrier")): return [("lookup_records", LookupQuery(record_type="order",record_id=oid,include_related=True))]
